...
few lines of text (see this file in raw mode to see how) and using the same key
combination over those lines. Thus, the only manual step is to copy/paste those
tables from that file into this one, and with only two key combinations for
table/graph the job is done.
